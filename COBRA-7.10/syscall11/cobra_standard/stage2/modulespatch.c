...
SprxPatch emulator_api_patches[] =
{
	...
	{ psp_read+0x20, LI(R3, SYSCALL11_OPCODE_READ_PSP_UMD), &condition_psp_iso },	
	...
	{ psp_read+0x58, LI(R3, SYSCALL11_OPCODE_READ_PSP_HEADER), &condition_psp_iso },	
...
SprxPatch pemucorelib_patches[] =
{
#ifdef FIRMWARE_3_55
#ifdef DEBUG
	{ psp_debug_patch, LI(R3, SYSCALL11_OPCODE_PSP_SONY_BUG), &condition_psp_iso },
	...
	{ psp_prx_patch+0x18, LI(R3, SYSCALL11_OPCODE_PSP_PRX_PATCH), &condition_psp_iso },
	...
	{ psp_prx_patch+0x48, LI(R3, SYSCALL11_OPCODE_PSP_POST_SAVEDATA_INITSTART), &condition_psp_iso },
	...
	{ psp_prx_patch+0x70, LI(R3, SYSCALL11_OPCODE_PSP_POST_SAVEDATA_SHUTDOWNSTART), &condition_psp_iso },
...
SprxPatch libfs_external_patches[] =
{
	...
	{ aio_copy_root_offset+0x14, LI(R3, SYSCALL11_OPCODE_AIO_COPY_ROOT), &condition_apphome },
	...
	// { DPRINTF("Offset: 0x%08X | Data: 0x%08X\n", aio_copy_root_offset, condition_apphome) },
	{ 0 }
};
...
uint8_t condition_vsh_check = 1;
uint8_t condition_toggle_psxemu = 1;
		...
		for (int i = 0; i < N_PATCH_TABLE_ENTRIES; i++)
		{
			if (patch_table[i].hash == hash)
			{
				if (condition_vsh_check)
				{
					...
					{
						case VSH_HASH:
							DPRINTF("Now patching spoofed DEBUG %s %lx\n", hash_to_name(hash), hash);
						break;

						case VSH_NRM_HASH:
							DPRINTF("Now patching normal DEBUG %s %lx\n", hash_to_name(hash), hash);
						break;

						case VSH_CEX_HASH:
							DPRINTF("Now patching spoofed RETAIL %s %lx\n", hash_to_name(hash), hash);
						break;
	...
	if (get_call_address(1) == (void *)MKA(process_map_caller_call))
	{
		if (process_addr == 0x10000 && size == vsh_text_size && flags == 0x2008004)
		{
			if (condition_vsh_check)
			{
				...
				{
					case VSH_HASH:
						DPRINTF("Making spoofed DEBUG VSH text writeable, Size: 0x%lx\n", size);
					break;

					case VSH_NRM_HASH:
						DPRINTF("Making normal DEBUG VSH text writeable, Size: 0x%lx\n", size);
					break;

					case VSH_CEX_HASH:
						DPRINTF("Making spoofed RETAIL VSH text writeable, Size: 0x%lx\n", size);
					break;
...
LV2_HOOKED_FUNCTION_PRECALL_SUCCESS_8(int, load_process_hooked, (process_t process, int fd, char *path, int r6, uint64_t r7, uint64_t r8, uint64_t r9, uint64_t r10, uint64_t sp_70))
{
	...
	else if (strcmp(path, "/dev_hdd0/game/BLES80608/USRDIR/RELOAD.SELF") == 0)
	{
		// Block peek in multiman so that it detects it is cobra and enters mmcm mode;
			block_peek = 1;
	}
...
/* void do_spoof_patches(void)
{
	...
	if (config.spoof_version && config.spoof_revision)
	{
		...
		patch[0] = MR(R4, R27);
		patch[1] = LI(R11, 0xB);
		patch[2] = LI(R3, SYSCALL11_OPCODE_VSH_SPOOF_VERSION);
		patch[3] = SC;	
...
void load_boot_plugins(void)
{
	...
	//Loading webman from flash - must first detect if the toggle is activated
	if ((prx_load_vsh_plugin(current_slot, PRX_PATH, NULL, 0) >=0) || (prx_load_vsh_plugin(current_slot, PRX_TEST, NULL, 0) >=0))
	{
	...
	while (num_loaded < MAX_BOOT_PLUGINS)
	{
		...
		{
			//KW BEGIN
			if ((webman_loaded != 1) || (!strcmp(path, "webMAN.sprx")) || (!strcmp(path, "webMAN_test.sprx"))) 		
			{
